...
var margin = {top: 20, right: 20, bottom: 100, left: 40},
	...
	var xxxs = d3.formatSpecifier("f");
	xxxs.precision = d3.precisionFixed(0.01);
	var xxf = d3.format(xxxs);	
...
if(theWidth < 400){
	...
	.attr('transform', 'translate(' + margin.left + ',' + (margin.top + 60 + 90) + ')');

	var context = svg.append('g')
	.attr('class', 'context')
	.attr('transform', 'translate(' + margin2.left + ',' + (margin2.top + 60 + 90) + ')');
	...
	d3.csv('data/BB.TO.csv', type, function(err, data) {
		...
		function mousemove() {
			...
			var ttavg = ((d.high+d.low)/2);
			helperText.text(legendFormat(new Date(d.date)) + ' - Price: ' + d.price + ' Avg: ' + xxf(ttavg) + ' Volume: ' + d.volume);
			...
			averageTooltip.attr('transform', 'translate(' + x(d.date) + ',' + y(ttavg) + ')');
